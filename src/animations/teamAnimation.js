...
function teamAnimation() {
  ...
  document.querySelectorAll(".gradient-hover").forEach((element) => {
    element.addEventListener("mousemove", (e) => {
      ...
      element.style.setProperty(
        "background",
        `linear-gradient(${angleDeg}deg, #262262 6.24%, #52af5d 91.17%, #48af55 91.17%)`
      );
    });
  });
  ...
  function td(slider, data, f, two = false) {
    ...
    let len = data.length,
      current = len - 1,
      closedWidth = Math.floor(
        window.innerWidth / (f > 10)
          ? 50 * dataExec.length
          : 50 * dataHeads.length
      );
    for (var i = 0; i < len; i++) {
      ...
      if (f > 10) {
        if (two) {
          inhtmlclass = "headdesg2";
        } else {
          inhtmlclass = "headdesg";
        }
      } else {
      ...
      b.onmouseenter = b.onclick = (e) => {
        ...
        gsap.to(".box" + twovar + f, {
          ...
          left: (i) =>
            i <= current
              ? i * closedWidth
              : (f <= 10 ? slider1.offsetWidth : slider2.offsetWidth) -
                (len - i) * closedWidth,
          x: 0,
